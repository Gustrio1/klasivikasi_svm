...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            ...
            <table class="table-base w-full">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        <td class="table-td font-bold text-gray-800">{{ $row['nama'] }}</td>
                        <td class="table-td text-center">
                            <span class="text-xs font-semibold {{ $row['kelas_asli'] == 1 ? 'text-emerald-700' : 'text-red-600' }}">
                                {{ $row['label_asli'] }}
                            </span>
                        </td>
                        <td class="table-td text-center">
                            <span class="text-xs font-semibold {{ $row['kelas_prediksi'] == 1 ? 'text-emerald-700' : 'text-red-600' }}">
                                {{ $row['label_prediksi'] }}
                            </span>
                        </td>
                        <td class="table-td text-center font-mono text-xs {{ $row['fx'] > 0 ? 'text-emerald-700' : 'text-red-600' }}">
                            {{ $row['fx'] > 0 ? '+' : '' }}{{ $row['fx'] }}
                        </td>
                        <td class="table-td text-center">
                            @if($row['benar'])
    ...
    <div class="space-y-4">
        ...
        {{-- Pie Chart Prediksi Benar vs Salah --}}
        <div class="card">
            ...
            <div class="flex items-center gap-4">
                ...
                <div class="space-y-2 text-sm">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="font-black text-gray-800 ml-auto">{{ $benar }}</span>
                    </div>
                    <div class="flex items-center gap-2">
                        <span class="w-3 h-3 rounded-sm bg-red-400 shrink-0"></span>
                        <span class="text-gray-600">Prediksi Salah</span>
                        <span class="font-black text-gray-800 ml-auto">{{ $salah }}</span>
                    </div>
                    <hr class="border-gray-100">
                    <div class="flex items-center gap-2">
                        <span class="w-3 h-3 rounded-sm bg-indigo-500 shrink-0"></span>
                        <span class="text-gray-600">Akurasi</span>
                        <span class="font-black text-indigo-700 ml-auto">{{ $akurasi }}%</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    new Chart(document.getElementById('chartAkurasi'), {
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    callbacks: {
                        label: ctx => ` ${ctx.label}: ${ctx.raw} data`
                    }
                }
            }
        }
