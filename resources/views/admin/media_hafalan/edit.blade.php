...
@section('title', 'Edit Media Hafalan (Admin)')

@section('content')

<x-page-header
    title="Edit Media Hafalan"
    subtitle="Perbarui data media hafalan yang akan direkomendasikan kepada siswa."
    :links="[
        ['label' => 'Dashboard', 'url' => route('dashboard')],
        ['label' => 'Media Hafalan', 'url' => route('admin.media-hafalan.index')],
        ['label' => 'Edit Media', 'url' => null],
    ]"
/>

<div class="max-w-5xl mx-auto pb-12" x-data="{ jenis_media: '{{ old('jenis_media', $mediaHafalan->jenis_media) }}' }">
    <div class="card p-8 border-none shadow-2xl">
        <form action="{{ route('admin.media-hafalan.update', $mediaHafalan->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-10 max-w-2xl mx-auto">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="nama_media" value="{{ old('nama_media', $mediaHafalan->nama_media) }}" 
                    ...
                    <div x-show="jenis_media === 'digital'" style="display: none;" x-transition>
                        ...
                        <input type="url" name="url_link" value="{{ old('url_link', $mediaHafalan->url_link) }}" 
            ...
            <div class="mt-12 flex justify-end gap-3 border-t border-gray-100 pt-8">
                <a href="{{ route('admin.media-hafalan.index') }}" class="btn-secondary">Batal</a>
                <button type="submit" class="p-3 bg-amber-500 text-white rounded-xl shadow-lg shadow-amber-200 font-black px-8 hover:bg-amber-600 transition">Update Media</button>
            </div>
        </form>
    </div>
</div>
